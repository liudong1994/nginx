...
struct ngx_event_s {
    /*
        事件相关的对象 通常data都是指向ngx_connection_t连接对象
        开启文件异步I/O时 它可能会指向ngx_event_aio_t结构体
    */
    void            *data;

    //标志位 为1时表示事件是可写的 TCP连接目前状态是可写的 连接处于可以发送网络包的状态
    unsigned         write:1;

    /*
        标志位 为1时表示此事件可以建立新的连接 在ngx_cycle_t中的listening动态数组中
    ...
    */
    ...
    /*
        ...
        这时 可通过instance标志位来避免处理后面的已经过期的事件 这是一个巧妙的设计方法
    */
    unsigned         instance:1;
     ...
     */
     /*
        ...
        这个状态对应着事件驱动模块处理方式的不同 在使用事件时，一般不会直接改变active标志位
     */
    unsigned         active:1;

    
    //标志位 为1时表示禁用事件 仅在kqueue或者rtsig事件驱动模块中有效 对epoll事件驱动模块无意义 
    unsigned         disabled:1;

    /* the ready event; in aio mode 0 means that no operation can be posted */
    /*
        标志位 为1时表示当前事件已经准备就绪 允许这个事件的消费模块处理这个事件
        在HTTP框架中 经常会检查事件的ready标志位以确定是否可以接收请求或者发送响应
    */
    unsigned         ready:1;

    //该标志位仅对kqueue、eventport等模块有意义 对于epoll事件驱动模块无意义
    unsigned         oneshot:1;
    ...
    //标志位 为1时表示这个事件已经超时 提示事件的消费模块做超时处理
    unsigned         timedout:1;
    ...
    /*
        标志位 为1时表示延迟建立TCP连接 经过TCP三次握手后并不建立连接
    ...
    */
    ...
    /*
        post事件将会构成一个队列再统一处理 这个队列以next和prev作为链表指针 以此构成一个简易的双向链表 
...
typedef struct {
    ...
    */
    ...
    //仅在多线程环境下会被调用 Nginx在产品环境下还不会以多线程方式运行
...
#if (NGX_DEBUG)
	/*
		在—with-debug编译模式下 可以仅针对某些客户端建立的连接输出调试级别的日志 
