...
typedef struct {
    ...
    /*
        接收HTTP包体的缓冲区链表 当包体需要全部存放在内存中时
        如果一块ngx_buf_t缓冲区无法存放完 这时就需要使用ngx_chain_t链表来存放
    */
    ngx_chain_t                      *bufs;
    // 直接接收HTTP包体的缓存
    ngx_buf_t                        *buf;
...
struct ngx_http_request_s {
    ...
    // 指向存放所有HTTP模块的上下文结构体的指针数组(每个HTTP模块最多一个)
    void                            **ctx;
    ...
    /*
        ...
        但如果该方法无法一次处理完该请求的全部业务 在归还控制权到epoll事件模块后
        该请求再次被回调时 将通过ngx_http_request_handler方法来处理
        而这个方法中对于可读事件的处理就是调用read_event_handler处理请求
    ...
    */
    ngx_http_event_handler_pt         write_event_handler;	//subrequest父请求重新激活后的回调方法
    ...
    */
    ...
    ngx_http_post_subrequest_t       *post_subrequest;	//用途：1.辨别是否有子请求
    /*
        所有的子请求都是通过posted_requests这个单链表来链接起来的 
        执行post子请求时调用的ngx_http_run_posted_requests方法就是通过遍历该单链表来执行子请求的
    */
    ...
    */
    ...
    /*
        在NGX_HTTP_ACCESS_PHASE阶段需要判断请求是否具有访问权限时 
    ...
    */
    ...
    /*
        ...
        又如 当我们接收HTTP包体时 由于这也是一个异步调用 所以count上也需要加1
        这样在结束请求时 就不会在count引用计数未清零时销毁请求 可以参见ngx_http_close_request方法
    */
    unsigned                          count:8;
    // 阻塞标志位 目前仅由aio使用
    unsigned                          blocked:8;
    ...
    */
    ...
    /*
        延迟关闭标志位 为1时表示需要延迟关闭 
        例如 在接收完HTTP头部时如果发现包体存在 该标志位会设为1 而放弃接收包体时则会设为0
    */
    ...
    /*
        ...
        若已经成功地启动响应头部发送流程 该标志位就会置为1 用来防止反复地发送头部
    */
